...
const Color: FC<props> = (props) => {
	...
	const updateHome = async () => {
		const res = await axios.patch(`/api/workspace/${workspace.groupId}/settings/general/premium`, { 
			widgets: workspace.settings.widgets,
		 });
		if (res.status === 200) {
			triggerToast.success("Updated premium");
		} else {
			triggerToast.error("Failed to update premium");
		}
	};
	...
	return (
		<div>
			<p> Upgrade microdot with a premium subscription license </p>
			<div className="">
				...
				<Button classoverride="mt-4" onPress={() => updateHome()}> Upgrade </Button>
